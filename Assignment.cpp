...
void addtopnode(char *renk, char *tip, char *deger, char *down);
void addboardnode(char *renk, char *tip, char *deger, char *down, int order);
void addheaderboardnode(int i);
void addheaderfoundationnode(char *tip);
void listeletopnode();
void listeleboardnode();
void listelefoundationnode();
void dosyadanoku();
bool topToFoundation(char *, char *, char *);
bool addfoundationnode(topnode *ekle);
bool addboardnode2(topnode *ekle, int d);
bool topToBoard(char *a, char *b, char *c, int d);
bool boardToBoard(char *a, char *b, char *c, int d, int e);
bool addboardtoboard(boardnode *ekle, int e);
...
void addtopnode(char *renk, char *tip, char *deger, char *down)			//add function to doubly list
{
	topnode *temp = new topnode;
	strcpy(temp->color, renk);
	strcpy(temp->suit, tip);
	strcpy(temp->symbol, deger);
...
bool addfoundationnode(topnode *ekle)				// top to foundation add
{
	char deger[6];
	int say = 1;
	if (strcmp(ekle->symbol, "A") == 0)					// converts sysbols to number in order to easy check
		strcpy(deger, "1");
	else if (strcmp(ekle->symbol, "J") == 0)
		strcpy(deger, "11");
	else if (strcmp(ekle->symbol, "Q") == 0)
		strcpy(deger, "12");
	else if (strcmp(ekle->symbol, "K") == 0)
		strcpy(deger, "13");
	else
		strcpy(deger, ekle->symbol);
	int asagiilerle = atoi(deger);
	int sagailerle;

	if (strcmp(ekle->suit, "S") == 0)
		sagailerle = 1;
	else if (strcmp(ekle->suit, "H") == 0)
		sagailerle = 2;
	else if (strcmp(ekle->suit, "D") == 0)
		sagailerle = 3;
	else if (strcmp(ekle->suit, "C") == 0)
		sagailerle = 4;
	...
	strcpy(temp->suit, ekle->suit);
	strcpy(temp->color, ekle->color);
	strcpy(temp->symbol, ekle->symbol);
	temp->next = NULL;
	temp->bottom = NULL;
	traversedown->bottom = temp;
	return true;
}
bool addfoundationnode2(boardnode *ekle)				// BOARD to foundation add
{
	char deger[6];
	int say = 1;
	if (strcmp(ekle->symbol, "A") == 0)					// converts sysbols to number in order to easy check
	...
	if (strcmp(ekle->suit, "S") == 0)							//in order to track list
		sagailerle = 1;
	else if (strcmp(ekle->suit, "H") == 0)
		sagailerle = 2;
	else if (strcmp(ekle->suit, "D") == 0)
...
bool addboardnode2(topnode *ekle, int d)            // top to board add
{
	char deger[6];
	char deger2[6];
	int say = 1;
	if (strcmp(ekle->symbol, "A") == 0)
		strcpy(deger, "1");
	else if (strcmp(ekle->symbol, "J") == 0)
		strcpy(deger, "11");
	else if (strcmp(ekle->symbol, "Q") == 0)
	...
	if (strcmp(traversedown->symbol, "A") == 0)
		...
		strcpy(deger2, traversedown->symbol);
	int kartdegeri = atoi(deger2);

	if (traverse->bottom == NULL && ekledegeri == 13)
	{
	...
	else if (traverse->bottom == NULL && ekledegeri != 13)		
		return false;
	

	if (!(kartdegeri == ekledegeri + 1))								//card must 1 less from upper card and different color
		return false;
	
	if (!(strcmp(ekle->color,traversedown->color)!=0))
	{
		return false;
	}
...
bool addboardtoboard(boardnode *ekle, int e)
{
	...
	if (traverse->bottom == NULL && ekledegeri == 13)			//if list is empty you can add King
	{
		boardnode *temp = new boardnode;
		strcpy(temp->suit, ekle->suit);
		strcpy(temp->color, ekle->color);
		...
		strcpy(temp->turned, "Up");
		temp->next = NULL;
		temp->bottom = ekle->bottom;
		traversedown->bottom = temp;
		return true;
	}
	else if (traverse->bottom == NULL && ekledegeri != 13)			//yeni eklendi.  if list is empty you cannot add other than king 
		return false;
...
void addboardnode(char *renk, char *tip, char *deger, char *down, int order)	 //file to board multilist
{
	boardnode *temp = new boardnode;
	strcpy(temp->color, renk);
	strcpy(temp->suit, tip);
...
void listeletopnode()									// list top node
{
